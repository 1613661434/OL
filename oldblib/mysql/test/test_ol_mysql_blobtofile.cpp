...
int main(int argc, char* argv[])
{
    ...
    // 注意连接字符串的格式需要与connecttodb方法要求一致
    ...
    unsigned long blob_length = sizeof(blob_buffer);

    // 绑定BLOB字段（位置1，缓冲区，长度）- 修复参数太少的错误
    ...
    // 将BLOB字段内容写入文件（只传文件名参数）- 修复参数太多的错误
    string output_file = R"(C:\test\data\pic_out.jpeg)";
    if (stmt.blobtofile(output_file) != 0)
    {
        printf("BLOB字段写入文件失败：%s\n", stmt.message().c_str());
        return -1;
    }

    printf("成功将数据库BLOB字段提取到文件：%s\n", output_file.c_str());
