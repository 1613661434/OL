/*
 *  程序名：test_ol_oci_clobtofile.cpp，演示从数据库读取CLOB字段并保存到文件
 *  作者：ol
 */
...
int main(int argc, char* argv[])
{
    ...
    // 将CLOB字段内容写入文件
    const string filename = "/home/mysql/OL/ol_database/oracle/test/data/memo_out.txt";
    if (stmt->clobtofile(1, filename) != 0)
    {
        printf("CLOB字段写入文件失败：%s\n", stmt->errorMsg().c_str());
        return -1;
    }
