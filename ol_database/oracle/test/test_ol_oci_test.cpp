/****************************************************************************************/
/*
 * 程序名：test_ol_oci_test.cpp
 * 功能描述：Oracle连接池综合测试用例，测试内容包括：
 *          - 单连接基础操作（连接、增删改查、事务）
 *          - 预处理语句（绑定参数、执行、结果集）
...
// 全局测试配置
const std::string ORACLE_CONN_STR = "scott/000888@snorcl11g_5";
const std::string ORACLE_CHARSET = "Simplified Chinese_China.AL32UTF8";
...
bool testSingleDBConn()
{
    try
    {
        ol::oracle::DBConn conn;
        // 配置连接参数
        conn.setConnectParam(ORACLE_CONN_STR, ORACLE_CHARSET, true);
        ...
        // 3. 测试插入数据
        ret = conn.execute("INSERT INTO test_pool_table (id, name, age) VALUES (1, 'test_user1', 20)");
        ...
        ret = conn.execute("INSERT INTO test_pool_table (id, name, age) VALUES (2, 'test_rollback', 30)");
        if (ret != 0)
        {
...
/**
 * @brief 测试2：Oracle预处理语句（stmt）功能
 * @param conn 已建立连接的Oracle连接对象
 * @return 测试全部通过返回true，任意步骤失败返回false
 * @note 测试内容：SQL预处理、参数绑定、增查操作、结果集获取
 */
bool testDBStmt(ol::oracle::DBConn& conn)
{
    try
    {
        // 1. 创建预处理语句对象
        ...
        // 2. 预处理插入SQL
        const char* insertSql = "INSERT INTO test_pool_table (id, name, age) VALUES (:1, :2, :3)";
        ...
        long age = 25;
        if (stmt->bindin(1, id) != 0 || stmt->bindin(2, name, nameLen) != 0 || stmt->bindin(3, age) != 0)
        {
        ...
        const char* querySql = "SELECT age FROM test_pool_table WHERE name = :1";
        ...
        if (queryStmt->bindin(1, name, nameLen) != 0)
        {
...
bool testDBPool()
{
    ...
    {
        // ===================== 适配单例模式：获取全局唯一连接池 =====================
        auto& pool = ol::DBPool<ol::oracle::DBConn>::GetInst();
...
bool cleanTestEnv()
{
    try
    {
        ...
        // 删除测试表
        int ret = conn.execute("DROP TABLE test_pool_table PURGE");
        if (ret != 0)
        {
...
/**
 * @brief 主函数：执行所有Oracle综合测试用例
 * @return 程序退出码
 */
int main()
{
    std::cout << "==================== Oracle连接池综合测试 ====================" << std::endl;

    // 顺序执行所有测试用例
    printTestResult("单个Oracle连接基本操作", testSingleDBConn());
